...
const ThankYouPage = async ({ searchParams }: PageProps) => {
  ...
  const user = await $currentUser(nextCookies);
  ...
  const [ order ] = orders      // getting the first order from multiple orders to get the userId.

  if (!order) return notFound()

  const orderUserId = typeof order.user === 'string' ? order.user : order.user.id;

// Not authorized user or user that is not logged in.l
